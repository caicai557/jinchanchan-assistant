...
Windows 版本需要 ADB (Android Debug Bridge) 与模拟器通信。支持两种模式：

| 模式 | 说明 | 适用场景 |
|------|------|----------|
| 内置 platform-tools | 打包时包含 adb.exe | 开箱即用，体积增大约 5MB |
| 外部 ADB | 运行时检测系统 ADB | 体积小，需用户自行安装 |
...
```
1. 检查 PATH 环境变量中的 adb
2. 检查常用模拟器路径:
   - 雷电: C:\leidian\LDPlayer9\adb.exe
   - 夜神: C:\Nox\bin\nox_adb.exe
   - MuMu: C:\Program Files\Netease\MuMuPlayer*\shell\adb.exe
3. 如果未找到，输出友好提示
```
...
=============================================================
  ...
  2. 使用模拟器自带的 ADB:
     - 雷电模拟器: 内置 adb.exe
     - 夜神模拟器: 内置 nox_adb.exe
     - MuMu 模拟器: 内置 adb.exe

  3. 将 ADB 目录添加到 PATH 环境变量
